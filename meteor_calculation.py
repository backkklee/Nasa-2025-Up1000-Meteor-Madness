...
def fetch_elements(designation):
    url = f"https://ssd-api.jpl.nasa.gov/sbdb.api?des={designation}"
    resp = requests.get(url)
    data = resp.json()
    orbit = data['orbit']
    elements = {
        'a': orbit['elements']['a'],     # semi-major axis [au]
        'e': orbit['elements']['e'],     # eccentricity
        'i': orbit['elements']['i'],     # inclination [deg]
        'om': orbit['elements']['om'],   # longitude of ascending node [deg]
        'w': orbit['elements']['w'],     # argument of perihelion [deg]
        'ma': orbit['elements']['ma'],   # mean anomaly at epoch [deg]
        'epoch': orbit['epoch'],         # epoch [Julian date]
    }
    return elements
...
def fetch_elements(des):
    url = f'https://ssd-api.jpl.nasa.gov/sbdb.api?des={des}'
    try:
        resp = requests.get(url, timeout=5)
        if resp.status_code != 200:
            return None
        orb = resp.json().get('orbit', {}).get('elements', {})
        return {
            'om': orb.get('om'),   # longitude of ascending node [deg]
            'w': orb.get('w'),     # argument of perihelion [deg]
            'ma': orb.get('ma'),   # mean anomaly [deg]
            'epoch': orb.get('epoch') # epoch [Julian date]
        }
    except Exception as e:
        print(f"Error fetching {des}: {e}")
        return None
...
for i, row in df.iterrows():
    des = row['designation']
    new_data = fetch_elements(des)
    if new_data:
        all_new_data.append(new_data)
        time.sleep(0.2)  # Respectful API usage
    else:
        all_new_data.append({'om': None, 'w': None, 'ma': None, 'epoch': None})

new_df = pd.concat([df, pd.DataFrame(all_new_data)], axis=1)
new_df.to_csv('asteroid_data_full.csv', index=False)
